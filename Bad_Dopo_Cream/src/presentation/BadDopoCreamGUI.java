...
public class BadDopoCreamGUI extends JFrame {
    ...
    private domain.BadDopoCream game;
    ...
    public BadDopoCreamGUI() {
        ...
        this.game = new domain.BadDopoCream();
    ...
    private void showMainMenu() {
        MenuPanel menuPanel = new MenuPanel("resources\\Fondo Inicio.png");
    ...
    public static void main(String[] args) {
        try {
            ...
            domain.BadDopoCream_Exceptions.GameInitializationException ex = 
                new domain.BadDopoCream_Exceptions.GameInitializationException(
                    "No se pudo configurar el Look and Feel del sistema", e);
    ...
    private void showLevelSelection(String gameMode, String player1Character, String player2Character, String enemyCharacter) {
        ...
        // Cuando se selecciona un nivel, ir al juego
        levelScreen.setLevelSelectedListener(levelNumber -> {
            showGameScreen(gameMode, player1Character, player2Character, enemyCharacter, levelNumber);
        });
    ...
    /**
     * Muestra la pantalla de juego con los personajes seleccionados
     * @param gameMode Modo de juego ("Normal", "PvP-Cooperativo", "Machine-vs-Machine")
     * @param player1Character Personaje del jugador 1 (helado)
     * @param player2Character Personaje del jugador 2 (helado en cooperativo, null en otros modos)
     * @param enemyCharacter Personaje enemigo controlado por jugador (solo en modo Versus)
     * @param levelNumber Número del nivel seleccionado
     */
    private void showGameScreen(String gameMode, String player1Character, String player2Character, String enemyCharacter, int levelNumber) {
        ...
        // IMPORTANTE: Crear nueva instancia del juego para limpiar estado anterior
        game = new domain.BadDopoCream();
        
        // Inicializar el juego en el backend
        initializeGameInBackend(gameMode, player1Character, player2Character, enemyCharacter, levelNumber);
     ...
     * Si el juego está ganado (victoria total), reinicia desde nivel 1
     * Si es game over, reinicia el nivel actual
     */
    private void restartGame() {
        ...
        game = new domain.BadDopoCream();  // Crear nueva instancia
     ...
     */
    public void loadGameFromState(domain.GameState state) {
    ...
    private void initializeGameInBackend(String gameMode, String player1Character, 
        ...
        domain.IceCream player1 = createIceCream(player1Character, new domain.Location(2, 2));
        
        // Usar LevelConfigurator para crear el nivel "quemado"
        domain.Level level = domain.LevelConfigurator.createLevel(levelNumber, player1);
        
        // En modo cooperativo, agregar segundo jugador
        if("Cooperative".equals(gameMode) && player2Character != null){
            domain.IceCream player2 = createIceCream(player2Character, new domain.Location(4, 2));
     ...
     */
    private domain.Level createLevel(int levelNumber) {
        // Por ahora, crear un nivel de prueba
        // TODO: Implementar configuraciones específicas para cada nivel
        domain.Level level = new domain.Level(levelNumber, 20, 15);
        
        // Agregar paredes de ejemplo
        for (int x = 0; x < 20; x++) {
            level.addWall(new domain.Location(x, 0));
            level.addWall(new domain.Location(x, 14));
        }
        for (int y = 1; y < 14; y++) {
            level.addWall(new domain.Location(0, y));
            level.addWall(new domain.Location(19, y));
        }
        
        // Agregar algunas paredes internas
        for (int x = 5; x < 10; x++) {
            level.addWall(new domain.Location(x, 7));
        }
        
        // Agregar frutas de ejemplo
        level.addFruit(new domain.Banana(new domain.Location(3, 3)));
        level.addFruit(new domain.Cherry(new domain.Location(16, 3)));
        level.addFruit(new domain.Grapes(new domain.Location(3, 11)));
        level.addFruit(new domain.Pineapple(new domain.Location(16, 11)));
        
        // Agregar enemigos de ejemplo
        level.addEnemy(new domain.Maceta(new domain.Location(10, 5)));
        level.addEnemy(new domain.Troll(new domain.Location(10, 10)));
     ...
     */
    private domain.IceCream createIceCream(String characterName, domain.Location startLocation) {
        ...
        return new domain.IceCream(name, color, startLocation);
    }
     ...
     */
    public domain.BadDopoCream getGame() {
        return this.game;
    }
