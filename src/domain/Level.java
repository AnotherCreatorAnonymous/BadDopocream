...
public class Level {
    ...
    public void addFruit(Fruit fruit){
        ...
        // Configurar referencias al mapa para frutas que lo necesiten
        if (fruit.requiresMapReference()) {
            fruit.setMap(this.map);
        }
    }
    ...
    public boolean movePlayer(int dx, int dy){
        ...
        // Empujar piñas antes de mover
        pushPineapples(dx, dy, player.getLocation());
    ...
    public boolean movePlayer2(int dx, int dy){
        ...
        // Empujar piñas antes de mover
        pushPineapples(dx, dy, player2.getLocation());
    ...
    private void checkCactusCollisions(){
        for(Fruit fruit : fruits){
            if(!fruit.isCollected() && fruit.shouldKillPlayer(player.getLocation())){
                player.die();
                return;
            }
            if(player2 != null && !fruit.isCollected() && fruit.shouldKillPlayer(player2.getLocation())){
                player2.die();
                return;
            }
        }
    ...
    private void pushPineapples(int dx, int dy, Location playerLocation){
        for(Fruit fruit : fruits){
            if(!fruit.isCollected()){
                fruit.pushByPlayer(dx, dy, playerLocation);
            }
        }
    }
    ...
    public void moveEnemies(){
        ...
        for(Enemy enemy : enemies){
            ...
            if(collisionWithEnemy){
                ...
                // Cambiar dirección usando método polimórfico
                enemy.changeDirectionOnEnemyCollision(map);
                enemy.changeDirectionRandomly();
            }
    ...
    public void moveFruits(){
        for(Fruit fruit : fruits){
            ...
            // Actualizar comportamientos especiales usando polimorfismo
            fruit.update();
        }
    }
    ...
    /**
     * Rompe una línea de hielo en la dirección especificada (para player 1)
     */
    ...
    /**
     * Rompe una línea de hielo en la dirección especificada (para player 2)
     */
